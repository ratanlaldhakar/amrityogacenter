/*
=========================================
YOGAMRIT YOGA CENTER - 3D WEBGL GRAPHICS V2
=========================================
*/
...
window.updateThreeTheme = function(isLight) {
  if (isLight) {
    targetBgColor.setHex(0xfaf9f6); // Light Mode backdrop (warm cream-white)
  } else {
    ...
    if (mandalaGroup) {
      ...
      mandalaGroup.children.forEach(child => {
        if (child.userData.rotSpeed) {
          ...
          const ringBreath = Math.sin(time * child.userData.breathFreq) * 0.04;
          child.material.opacity = (0.18 - child.userData.breathFreq * 0.01) + ringBreath;
        }
      });
      ...
      chakraGroup.children.forEach(child => {
        if (child.userData.isChakra) {
          ...
          if (child.userData.chIdx === 6 || child.userData.chIdx === 3) {
            child.material.emissiveIntensity = 2.4 + Math.sin(time * 2.2) * 0.5;
          }
        }
      });
    }
