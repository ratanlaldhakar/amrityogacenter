...
const dbService = {
  ...
  async getGallery() {
    if (USE_SUPABASE && dbClient) {
      const { data, error } = await dbClient.from('gallery').select('*').order('id', { ascending: true });
      if (!error) return data;
    }
  ...
  async addGalleryItem(src, title, caption, category) {
    if (USE_SUPABASE && dbClient) {
      const { data, error } = await dbClient.from('gallery').insert([
        { src, title, caption, category }
      ]).select();
      if (!error && data && data.length > 0) {
        const db = getLocalDB();
        db.gallery.push(data[0]);
        saveLocalDB(db);
        return data[0];
      }
    }
  ...
  async updateGalleryItem(id, title, caption, category, src = null) {
    if (USE_SUPABASE && dbClient) {
      ...
      await dbClient.from('gallery').update(updateData).eq('id', id);
    }
    const db = getLocalDB();
