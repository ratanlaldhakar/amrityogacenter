...
async function renderDynamicContent() {
  ...
  if (heroWa) heroWa.href = `https://wa.me/91${hp.phone_primary}?text=Namaste! I would like to book a free trial class at Amrit Yoga Center.`;
...
function initThemeSwitcher() {
  ...
  // Set default theme to Dark Mode on first visit
  if (!savedTheme) {
...
function initTrialBookingSystem() {
  ...
  const toast = document.getElementById("trial-success-toast");
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      let dbRecord = null;
      try {
        dbRecord = await dbService.addTrialBooking(name, phone, age, gender, city, batch, goal, notes);
      } catch (dbErr) {
      ...
      if (typeof emailjs !== "undefined") {
        try {
          ...
          // Alert user that booking succeeded but email warning happened
          alert("Booking saved successfully! However, there was a minor issue sending the email notification to the studio.");
        }
      } else {
        console.warn("EmailJS library not loaded. Email notification skipped.");
        alert("Booking saved successfully! (Note: Email notifications are unavailable at the moment).");
      }

      submitBtn.disabled = false;
      submitBtn.innerHTML = originalHTML;

      // Show Gold Success Animation inside the Modal
      const benefitsBanner = document.querySelector("#trial-booking-modal .trial-benefits-banner");
      const successPanel = document.getElementById("trial-success-panel");
      ...
      setTimeout(() => {
        closeModal();
        // Restore elements visibility for subsequent bookings
        if (form) form.style.display = "block";
        if (benefitsBanner) benefitsBanner.style.display = "grid";
